...
summary = fetch_endpoint(f"{BASE_URL}/api/summary")
kpis = summary["kpis"]
...
assert kpis['total_records_final'] >= 7335, f"Expected at least 7,335 records, got {kpis['total_records_final']}"

print("\n=== 2. Testing Path Sanitization (Security Check) ===")
sources = fetch_endpoint(f"{BASE_URL}/api/datasets/sources")
...
final_lists = fetch_endpoint(f"{BASE_URL}/api/datasets/final")
...
dict_data = fetch_endpoint(f"{BASE_URL}/api/dictionary?page=1&page_size=200")
...
# Test dataset filter
dict_udise = fetch_endpoint(f"{BASE_URL}/api/dictionary?dataset=UDISE%2B%20National%20Schools%20Register")
...
dict_source = fetch_endpoint(f"{BASE_URL}/api/dictionary?field_classification=SOURCE%20FIELD")
...
# Test search query
dict_search = fetch_endpoint(f"{BASE_URL}/api/dictionary?search=intake")
...
gachi = fetch_endpoint(f"{BASE_URL}/api/search?q=Gachibowli")
...
# 2. KPHB -> JNTUH
kphb = fetch_endpoint(f"{BASE_URL}/api/search?q=KPHB")
